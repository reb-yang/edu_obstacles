...
student_data <- read_csv("student_data.csv")  %>% 
  mutate(age_group = ifelse(age_group == "15 to 24 years", "15 to 24 years", "25 and over")) %>% mutate(obstacles_future_edu = ifelse(obstacles_future_edu == "Other - Specify", "Other", obstacles_future_edu)) # loading data

# counting sex 
sex_table <- student_data %>% 
  group_by(sex) %>% 
  count() %>% 
  mutate(prop=paste(round(n/nrow(student_data), 2)*100, "%")) 
...
table <- rbind(sex_table, age_table %>% 
                 rename(sex = age_group), prv_table %>% rename(sex = province)) 
colnames(table) <- c("Variable", "Number of Respondents", "Proportion")
table <- table %>% 
  kbl(caption = "Student Demographics", align = c("lrr")) %>%
  pack_rows("Sex", 1, 2) %>%
  pack_rows("Age Group", 3, 4) %>% 
  pack_rows("Province", 5, 14) %>% 
...
likely_students <- student_data %>% 
  ...
  rename(edu_level = likely_future_edu) %>% mutate(prop = n*100/nrow(student_data))
...
desired_students <- student_data %>% 
  ...
  mutate(prop = 100*n/nrow(student_data))

future_education_data <- rbind(likely_students, desired_students)

# plot of proportions of students' desired and likely 

future_edu_props <-  future_education_data %>% 
  ggplot(aes(y =reorder(edu_level, prop ), 
             x=prop, fill = future_edu)) + 
  geom_bar(stat = "identity", position = "dodge") + 
  scale_fill_manual(values=mycols) + 
  theme_minimal()  + 
  labs(title = "Students' Desired vs. Likely Education Levels", 
       x="Percentage of Responses (%)", 
       y = "Education Level", 
       fill = "")  + 
  theme(plot.title = element_text(hjust = 0.5))  +
  geom_text(aes(label = paste(round(prop), "%")), 
            size = 3, 
            position = position_dodge(width = 1), 
            hjust = -0.1) + 
  scale_y_discrete( labels = c("Other", 
                               "High School Diploma/Equivalent",
                               "Non-college Certificate", 
                               "Professional (i.e. MD)" , 
                               "Ph.D." , 
                               "College/CEGEP", 
                               "Master's", 
...
future_edu_props <-  future_education_data %>% 
  ...
  geom_text(aes(label = paste(round(prop), "%")), 
            ...
            position = position_dodge(width = 1),
            hjust = -0.1) + 
  scale_y_discrete( labels = c("Other", "High School Diploma/Equivalent", 
                               "Non-college Certificate", 
                               "Professional (i.e. MD)" , 
                               "Ph.D." , 
                               "College/CEGEP", 
...
future_obstacles <- student_data %>%
  filter(obstacles != "NA") %>% 
  group_by(obstacles) %>%
  summarise(n = n(), prop = n*100/nrow(student_data)) %>% 
  ...
  theme(plot.title = element_text(hjust = 0.5))  + 
  geom_text(aes(label = paste(round(prop), "%")), size = 4, vjust = -0.25) + scale_y_continuous(limits = c(0,100))  
...
prv_obstacles <- cbind(student_data %>%
  ...
  theme_minimal()  +
  labs(x = "Percentage Facing Obstacles (%)", 
       y = "Province") + 
  ggtitle("Do You Face Obstacles in Obtaining Your Desired Education?") + 
  theme(plot.title = element_text(hjust = 0.5))  + 
  geom_text(aes(label = paste(round(prop), "%")), 
...
fem1<- nrow(student_data %>% 
              filter(sex == "Female", obstacles != "NA"))
male1 <- nrow(student_data %>%
                filter(sex == "Male", obstacles != "NA"))
sex1 <- student_data%>% 
  filter(obstacles!= "NA") %>% 
  group_by(sex, obstacles) %>%
  count() %>%
  summarise(prop = ifelse(sex=="Male", 100*n/male1, 100*n/fem1))

obstacles_sex_1 <- sex1 %>% 
  filter(obstacles == "Yes") %>% 
  ggplot(aes(y=sex, x = prop)) + 
  geom_bar(stat = "identity", width = 0.5, 
           fill = c("orchid2", "dodgerblue")) +
  theme_minimal() + 
  labs(title = "Are Women More Likely to Face Obstacles?", 
       y = "Sex", 
       x = "Percentage Facing Obstacles (%)") + 
...
sex2 <- student_data %>%
  ...
  count() %>% 
  summarise(prop = ifelse(sex=="Male", 100*n/male2, 100*n/fem2))

obstacles_sex_2 <- sex2 %>%
  ggplot(aes(y=reorder(obstacles_future_edu, prop), 
             x = prop,
             fill = sex)) + 
  geom_bar(stat = "identity", 
           position = "dodge") + 
...
young1<- nrow(student_data %>% 
                filter(age_group == "15 to 24 years", obstacles!= "NA"))
old1<- nrow(student_data  %>%
              filter(age_group  == "25 and over", obstacles!= "NA"))
age1 <- student_data  %>%
  filter(obstacles!= "NA") %>% 
  group_by(age_group, obstacles) %>%
  count() %>% 
  summarise(prop = ifelse(age_group == "15 to 24 years", 100*n/young1, 100*n/old1))
...
young2<- nrow(student_data  %>% 
                filter(age_group == "15 to 24 years", obstacles_future_edu != "NA"))
